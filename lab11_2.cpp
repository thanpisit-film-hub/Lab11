...
int drawCard(void){
    int _ScoreOfCard = rand()%13 + 1;
    return _ScoreOfCard;
}

int calScore(int x,int y,int z){
	int _x = x, _y = y, _z = z, _TrueScore, _sum;
	if(_x >= 10) _x = 10;
	if(_y >= 10) _y = 10;
	if(_z >= 10) _z = 10;
	_sum = _x + _y + _z;
...
int findYugiAction(int s){
	...
	if(s < 6) return 1; // Yugi will definitely draw (1) when current score (s) is less than 6
	if(s >= 6 and s < 9){
	    int _percent = rand() % 100;
		if(_percent >= 69) return 2;
		else return 1;
	}
	return 0;
}
